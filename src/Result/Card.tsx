		...
		return(
          <section className={classes.card} key={`section_${this.props.name}`}>
            <div className={classes.cardImg}
              ...
              key={`img_${this.props.name}`}></div>
            <div className={classes.cardContent} key={`content_${this.props.name}`}>
            <h2 className={classes.heading}>{this.props.name}</h2>
            <ul className={classes.features}>
              {(Object.keys(this.nameFields) as Array<keyof typeof this.nameFields>).map((elem) => {
                return (
                  <li className={classes.infoLine} key={`feature_${this.props.name}_${this.nameFields[elem]}`}>
                    <span className={classes.lineTitle}>{this.nameFields[elem]}</span>
                    {": "}
                    <span className={classes.lineValue}>{this.props[elem]}</span>
                  </li>
                )
              })}
            </ul>
